  ...
  return (
    <section className="d-flex flex-wrap gap-4 justify-content-between pe-5">
      ...
      <dl>
        <dt>Status</dt>
        <dd>
          <span className={`badge ${getTextBackgroundByStatus(order.status)}`}>{order.status}</span>
        </dd>
        <dt>Total</dt>
        <dd>{new Intl.NumberFormat("en-US", { style: "currency", currency: "USD" }).format(order.total)}</dd>
      </dl>
      <dl>
        <dt>Staff</dt>
        <dd>
