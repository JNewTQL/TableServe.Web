...
function SignInPage() {
  ...
  const signin: SubmitHandler<IAccount> = async (account) => {
    try {
      const { password: _, ...safeStaff } = await staffAPI.findByAccount(account.username, account.password);
  ...
  return (
    <main className="signin d-flex flex-column gap-4 justify-content-center align-items-center">
      <svg width={100} height={78} viewBox="0 0 78 32" fill="none" xmlns="http://www.w3.org/2000/svg">
