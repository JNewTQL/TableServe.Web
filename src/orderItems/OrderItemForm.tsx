    ...
    <form className="form w-50" onSubmit={handleSubmit(save)}>
      <div className="card p-4">
        ...
        <div className="mb-3">
          ...
          <select {...register("menuItemId", { valueAsNumber: true, required: "Menu item is required" })} className={`form-select ${errors?.menuItemId && "is-invalid"}`}>
        ...
        <div className="mb-3">
          ...
          <div className="form-label">{new Intl.NumberFormat("en-US", { style: "currency", currency: "USD" }).format(selectedMenuItem?.price ?? 0)}</div>
        </div>
        ...
        <div className="mb-3">
          ...
          <input id="notes" type="text" {...register("notes")} className="form-control" placeholder="Enter any notes for this item (optional)" />
        </div>

        <div className="mb-3">
          <label className="form-label">Amount</label>
          <div className="form-label">{new Intl.NumberFormat("en-US", { style: "currency", currency: "USD" }).format((selectedMenuItem?.price ?? 0) * quantity)}</div>
        </div>

        <div className="d-flex justify-content-end mt-4">
          <Link to={`/orders/detail/${orderId}`} className="btn btn-outline-primary me-2">
            Cancel
          </Link>
